...
2. Copy folder `Inventory` dari HP ini (`/sdcard/www/Inventory`) ke folder `C:\xampp\htdocs\` di komputer kantor.
3. Buka phpMyAdmin di komputer kantor (`http://localhost/phpmyadmin`):
   - Buat database baru bernama `db_inventory`.
   - Klik **Import** -> Pilih file `db_inventory.sql`.
...
3. Buka browser di HP Petugas (Chrome / Edge):
   - Buka alamat: `http://192.168.1.50/Inventory/app.php`
4. **Jadikan APK di HP**:
...
1. **Petugas di Pintu Masuk / Gudang (Via HP)**:
   - Begitu ada kiriman vendor/supplier datang, buka APK -> Klik **INPUT BARANG MASUK**.
...
3. **Admin / Pimpinan di Meja Kantor (Via Layar PC / Laptop)**:
   - Buka `http://localhost/Inventory/index.php`.
