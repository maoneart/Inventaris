<?php
// barang.php - Master Data Barang & Satuan Lengkap
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'tambah_barang') {
        ...
        if (empty($kode_barang) || empty($nama_barang)) {
            ...
            try {
                $stmt = $pdo->prepare("
                    INSERT INTO barang (kode_barang, barcode, nama_barang, id_kategori, id_satuan, stok_saat_ini, stok_minimum, lokasi_rak, spesifikasi)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$kode_barang, $barcode, $nama_barang, $id_kategori, $id_satuan, $stok_saat_ini, $stok_minimum, $lokasi_rak, $spesifikasi]);
                setFlash('success', 'Barang Ditambahkan', "Barang <strong>$nama_barang</strong> berhasil didaftarkan.");
...
// Generate Saran Kode Barang Baru
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
  <div>
    <h2 style="font-size: 1.25rem; font-weight: 800; color: #ffffff; display: flex; align-items: center; gap: 8px;">
      <i class="bi bi-boxes text-primary"></i> Master Data Barang & Inventaris
    </h2>
    <p style="font-size: 0.78rem; color: var(--text-muted);">Kelola katalog barang, satuan lengkap, stok minimum, dan lokasi rak</p>
  </div>
...
<div class="glass-card">
  <h3 style="font-size: 1rem; font-weight: 700; color: #60a5fa; margin-bottom: 14px; display: flex; align-items: center; gap: 6px;">
    <i class="bi bi-plus-square-fill"></i> Daftarkan Barang Baru
  </h3>
  ...
  <form action="barang.php" method="POST">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px;">
      ...
      <div class="form-group">
        ...
        <input type="text" name="spesifikasi" class="form-control" placeholder="Ukuran, voltase, grade material...">
      </div>
    </div>

    <div style="margin-top: 10px; display: flex; justify-content: flex-end;">
      <button type="submit" class="btn btn-primary">
        <i class="bi bi-save-fill"></i> Simpan Barang Baru
      </button>
    </div>
  </form>
</div>
...
<div class="glass-card">
  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; gap: 12px; flex-wrap: wrap;">
    <h3 style="font-size: 1rem; font-weight: 700; color: #ffffff;">Katalog Master Barang (<?= count($barangs) ?> Item)</h3>
    <input type="text" id="tableSearchInput" class="form-control" placeholder="🔍 Cari nama barang / kode..." style="max-width: 260px; padding: 6px 12px; font-size: 0.8rem;">
  </div>
    ...
    <table class="modern-table">
      ...
      <tbody>
        <?php foreach ($barangs as $b): ?>
          <tr>
            <td><strong style="color: #60a5fa;"><?= htmlspecialchars($b['kode_barang']) ?></strong></td>
            <td>
            ...
            <td><?= htmlspecialchars($b['singkatan']) ?> (<?= htmlspecialchars($b['nama_satuan']) ?>)</td>
            <td style="text-align: right; font-weight: 700; <?= $b['stok_saat_ini'] <= $b['stok_minimum'] ? 'color: #f87171;' : 'color: #34d399;' ?>">
